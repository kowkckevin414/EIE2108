...
def BVQCencode(in_image_filename, out_encoding_result_filename, d):
    ...
    for i in range(0,intensity.shape[0], d): # Column: Intensity
        for j in range(0,intensity.shape[1], d): # Row: Intensity
            ...
            # find index with shortest diseance
            bit = 0
            byte_carry= 0
...
fname_in = 'myTimg_encoded.out'
fname_out= 'myTimg_remake.png'
...
while True:
  try:
    ...
    print("Error404: not accpet other number")
...
BVQCencode(img_in, bvqc_out, d)
BVQCdecode(bvqc_out, img_out)
